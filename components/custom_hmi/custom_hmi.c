        ...
        switch (item->var_type)
        {
            ...
            case TYPENAME_LONG_INT:
                *(long*)item->variable += step;
                if ( *(long*)item->variable > item->max)    *(long*)item->variable = item->max;
                if ( *(long*)item->variable < item->min)    *(long*)item->variable = item->min;
            ...
            case TYPENAME_UNSIGNED_LONG_INT:
                *(unsigned int*)item->variable += step;
                if ( *(unsigned int*)item->variable > item->max)    *(unsigned int*)item->variable = item->max;
                if ( *(unsigned int*)item->variable < item->min)    *(unsigned int*)item->variable = item->min;
                set_unsigned_int_value(item->value_label, *(unsigned long*)item->variable);
                break;

            case TYPENAME_FLOAT:
                *(float*)item->variable += step/10;
                if ( *(float*)item->variable > item->max)    *(float*)item->variable = item->max;
                if ( *(float*)item->variable < item->min)    *(float*)item->variable = item->min;
                set_double_value(item->value_label, *(float*)item->variable, FLOAT_FORMAT);
                break;
            case TYPENAME_DOUBLE:
                *(double*)item->variable += step/10;
                if ( *(double*)item->variable > item->max)    *(double*)item->variable = item->max;
                if ( *(double*)item->variable < item->min)    *(double*)item->variable = item->min;
