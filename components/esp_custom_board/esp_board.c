...
{
    const gpio_config_t out_io_config = {
        .pin_bit_mask = BIT64(BSP_OUT0) | BIT64(BSP_OUT1),
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE
    };
    BSP_ERROR_CHECK_RETURN_ERR(gpio_config(&out_io_config));
    return ESP_OK;
}

esp_err_t bsp_out_set(const bsp_out_t out_io, const bool on)
{
    BSP_ERROR_CHECK_RETURN_ERR(gpio_set_level((gpio_num_t) out_io, (uint32_t) on));
    return ESP_OK;
}
...
esp_err_t bsp_display_new(const bsp_display_config_t *config, esp_lcd_panel_handle_t *ret_panel, esp_lcd_panel_io_handle_t *ret_io)
{
    ...
    ESP_ERROR_CHECK(esp_lcd_panel_set_gap(*ret_panel, 0, 24));
    return ret;
...
lv_disp_t *bsp_display_start(void)
{
    ...
    lv_disp_set_rotation(disp, true);
    return disp;
}
